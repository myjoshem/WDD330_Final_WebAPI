...
const getOne = async (req, res) => {
  try {
    const db = await mongodb.getDb();
    const hammockId = req.params.id;
    const result = await db.collection(dbTitle).findOne({ Id: hammockId });

    if (!result) {
      res.status(404).json({ error: 'Hammock not found' });
    } else {
      res.setHeader('Content-Type', 'application/json');
      res.status(200).json(result);
    }
...
const update = async (req, res) => {
  try {
    ...
    // Update the hammock
    const result = await db.collection(dbTitle).updateOne({ Id: hammockId }, { $set: req.body });

    // Check if the hammock was updated successfully
    if (result.matchedCount === 0) {
      return res.status(404).json({ error: 'Hammock not found' });
    }
...
const deleteItem = async (req, res) => {
  try {
    ...
    // Delete the hammock
    const result = await db.collection(dbTitle).deleteOne({ Id: hammockId });
